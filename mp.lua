...
imgui.OnInitialize(function()
    ...
    LogoImg = imgui.CreateTextureFromFile(getWorkingDirectory() .. '\\MP\\MP lua.png')
            ...
            if imgui.Button(u8"/ao",imgui.ImVec2(185,30)) then
                if ffi.string(inputMpName) == "" or ffi.string(inputMpWinner) == "" then
                    ...
                    if get_player(ffi.string(inputMpWinner)) then
                        if formaMp[0] then sampSendChat(string.format("a ao Победитель мероприятия \"%s\" становиться %s[%s]. Поздравляем!",u8:decode(ffi.string(inputMpName)),mpPlayer[1],mpPlayer[2]))
                        else sampSendChat(string.format("ao Победитель мероприятия \"%s\" становиться %s[%s]. Поздравляем!",u8:decode(ffi.string(inputMpName)),mpPlayer[1],mpPlayer[2]))
                        end
